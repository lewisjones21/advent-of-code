...
{
    private List<string> _wireA = new List<string>()
    {
    ...
    private List<string> _wireB = new List<string>()
    {
    ...
    {
        List<Tuple<Vector2, Vector2>> wireASegments, wireBSegments;
        Vector2 currentPos, nextPos;

        currentPos = Vector2.zero;
        nextPos = currentPos;
        wireASegments = new List<Tuple<Vector2, Vector2>>();
        for (int a = 0; a < _wireA.Count; a++)
        {
            nextPos = currentPos + GetDelta(_wireA[a]);
            wireASegments.Add(new Tuple<Vector2, Vector2>(currentPos, nextPos));
            currentPos = nextPos;
        }

        currentPos = Vector2.zero;
        nextPos = currentPos;
        wireBSegments = new List<Tuple<Vector2, Vector2>>();
        for (int b = 0; b < _wireB.Count; b++)
        {
            nextPos = currentPos + GetDelta(_wireB[b]);
            wireBSegments.Add(new Tuple<Vector2, Vector2>(currentPos, nextPos));
            currentPos = nextPos;
        }
    ...
    {
        int value = int.Parse(deltaString.Substring(1));
        switch (deltaString[0])
        {
            case 'R':
    ...
    private Vector2? GetIntersection(Tuple<Vector2, Vector2> segmentA,
                                     Tuple<Vector2, Vector2> segmentB)
    {
        // If the bounding ranges of the segments do not overlap
        if (Mathf.Max(segmentA.Item1.x, segmentA.Item2.x)
                < Mathf.Min(segmentB.Item1.x, segmentB.Item2.x)
            || Mathf.Max(segmentB.Item1.x, segmentB.Item2.x)
                < Mathf.Min(segmentA.Item1.x, segmentA.Item2.x)
            || Mathf.Max(segmentA.Item1.y, segmentA.Item2.y)
                < Mathf.Min(segmentB.Item1.y, segmentB.Item2.y)
            || Mathf.Max(segmentB.Item1.y, segmentB.Item2.y)
                < Mathf.Min(segmentA.Item1.y, segmentA.Item2.y))
        {
            return null;
        }
        // If segmentA is vertical
        if (segmentA.Item1.x == segmentA.Item2.x)
        {
            // If segmentB is vertical
            if (segmentB.Item1.x == segmentB.Item2.x)
            {
                return null;
            }
            return new Vector2(segmentA.Item1.x, segmentB.Item1.y);
        }
        // If segmentA is horizontal
        if (segmentA.Item1.y == segmentA.Item2.y)
        {
            // If segmentB is vertical
            if (segmentB.Item1.y == segmentB.Item2.y)
            {
                return null;
            }
            return new Vector2(segmentB.Item1.x, segmentA.Item1.y);
        }
        return null;
    }
